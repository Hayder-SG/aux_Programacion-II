...
class Fraternidad:
    ...
    def mostrar_info(self):
        print(f"\nFraternidad: {self.nombre}, Encargado: {self.encargado}")
...
class Gestion:
    ...
    def esta_2_fraternidad(self):
        ...
        repetidos = [bid for bid, cant in conteo.items() if cant > 1]

        if repetidos:
            for bailarin in self.bailarines:
                if bailarin.id in repetidos:
                    bailarin.mostrar_info()
        else:
    ...
    def agregar_bailarin(self, bailarin):
        for b in self.bailarines:
            if b.id == bailarin.id:
                print("ERROR: Ya existe un bailarín con ese ID.")
                return
...
g = Gestion(
    [b1, b2, b3, b4, b5, b6, b7],   # Todos los bailarines
    [fr1, fr2],                    # Las 2 fraternidades
    [f1, f2]                       # Las 2 facultades
)
...
g.agregar_bailarin(
    Bailarin("Bailarin 8", 27, 8, "Facultad 1", "Fraternidad 1")
)
